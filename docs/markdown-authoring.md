...
- **`toc`**: optional, default **`true`** (H1–H3). Controls the auto-generated table of contents in **`document`** layout. Ignored for **`slides`**. Page footers (**`n / total`** + eagle) behave like the normal document merge. Three accepted forms:
  - **`true`** / **`false`** — enable (H1–H3, default range) or skip the TOC entirely. Skipping bypasses the TOC pages and the measurement pass.
  - **integer 1–6** — shorthand for **`{ "minHeadingLevel": 1, "maxHeadingLevel": N }`**. For example, **`"toc": 4`** includes H1 through H4.
  - **object** **`{ "minHeadingLevel": <1–6>?, "maxHeadingLevel": <1–6>? }`** — explicit range. **`minHeadingLevel`** defaults to **`1`**, **`maxHeadingLevel`** to **`3`**. **`min` ≤ `max`** is enforced. Use **`{ "minHeadingLevel": 2 }`** when your **`#`** heading acts as a page-internal title and you don't want it in the outline.
...
- **`pageFormat`**: optional string, one of **`A3`**, **`A4`** (default), **`A5`**. **Single-valued for the whole document** — the title page always renders at the same paper size as the body. A brand that pins **`titlePage.pdfOptions.format`** in **`branding.json`** is silently overridden so the cover can't end up on a different sheet than the body. Unknown values fail fast with a clear error. Equivalent CLI flag: **`--page-format <fmt>`** — the CLI flag wins over the meta-level field.
...
| **`titlePage`**         | **`document`** / **`slides`**: **`pdfOptions`**, **`linkColor`**, layout **`…Style`** strings. Prefer **`min-height: 100%`** (or **`min-height: 100vh`**) on **`containerStyle`** / **`outerStyle`** so the title flexes to whichever paper size the user picks via **`meta.pageFormat`** / **`--page-format`**. **Avoid fixed-mm `min-height`** values (e.g. **`240mm`**) — they overflow on smaller paper formats and used to split the title across two sheets. md-pdf now ships a **`title-page-clamp.css`** that constrains **`body`** to **`100vh`** and clips overflow, so legacy mm values won't split anymore, but content past the page edge will be clipped instead of reflowed. With **`overflow: visible`** and **`display: flex; flex-direction: column`** on the container, long titles are not clipped. When **`pdfOptions.margin`** has **zero right** and **non‑zero left**, the engine adds an inner column (**`padding-right`** = left margin, **`padding-bottom`** reserves the footer/meta band, **`flex: 1 1 auto; min-height: 0`**). Optional **`contentColumnBottomPad`** (CSS length, e.g. **`"62mm"`**) adjusts that reserve. Optional **`contentColumnStyle`** replaces the whole inner wrapper (then you must supply matching flex/padding yourself if you keep the same footer layout). |
